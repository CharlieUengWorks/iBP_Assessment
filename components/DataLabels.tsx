...
export default function DataLabels(){
    ...
    let cpuTexts:string[]=['CPU'];
    let gpuTexts:string[]=['GPU'];
    ...
    if(data){
        ...
        if(cpuData.CoreTemperature&&cpuData.CoreTemperature>0)
            ...
            cpuTexts.push("TEMP:--");
        if(gpuData.name)
            gpuTexts.push(gpuData.name);
        else
            gpuTexts.push("--no name--");
        if(gpuData.CoreLoad)
            gpuTexts.push(`LOAD:${gpuData.CoreLoad}%`);
        else
            gpuTexts.push("LOAD:--");
        if(gpuData.CoreTemperature&&gpuData.CoreTemperature>0)
            gpuTexts.push(`TEMP:${gpuData.CoreTemperature}°C`);
        else
            gpuTexts.push("TEMP:--");
    }else{
